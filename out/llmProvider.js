...
async function askOpenAI(opts) {
    ...
    const systemPrompt = (0, systemPrompt_1.buildSystemPrompt)(toolPool, opts.planMode);
    ...
    const cleanMessages = messages.filter(m => !(m.role === 'tool' && m.toolCallId && !validToolIds.has(m.toolCallId)));
...
async function askAnthropic(opts) {
    ...
    const anthropic = new sdk_1.default({ apiKey, baseURL: baseUrl || undefined });
    const systemPrompt = (0, systemPrompt_1.buildSystemPrompt)(toolPool, planMode);
    ...
    // -- Non-streaming path --
    const response = await anthropic.messages.create(baseParams);
    const usage = {
...
function buildAnthropicMessages(messages) {
    ...
    for (const m of messages) {
        if (m.role === 'assistant' && m.toolCalls) {
            for (const tc of m.toolCalls) {
                allToolUseIds.add(sanitizeToolId(tc.id));
            }
        }
        ...
        if (m.role === 'tool') {
            ...
            if (lastMsg && lastMsg.role === 'user') {
                if (Array.isArray(lastMsg.content)) {
                    ...
                    lastMsg.content = [{ type: 'text', text: lastMsg.content }, block];
                }
            }
            else {
            ...
            if (lastMsg && lastMsg.role === m.role) {
                ...
                result.push({ role: m.role, content: blocks });
            }
            else {
