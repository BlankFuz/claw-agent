...
import { safeResolvePath } from './file';

// ---------------------------------------------------------------------------
// #20 — list_diagnostics
// ---------------------------------------------------------------------------

export const listDiagnosticsTool: ToolSpec = defineTool({
    name: 'list_diagnostics',
    category: 'search',
    description:
        'Get diagnostic errors and warnings from the workspace. Returns file paths, line numbers, ' +
    ...
    async execute(args, ctx) {
        ...
        if (filePath) {
            ...
            allDiagnostics = allDiagnostics.filter(([u]) =>
...
export const getDocumentSymbolsTool: ToolSpec = defineTool({
    name: 'get_document_symbols',
    category: 'search',
    description:
        'Get all symbols (functions, classes, variables, interfaces, etc.) in a file. ' +
    ...
    async execute(args, ctx) {
        ...
        const lines: string[] = [];
        const formatSymbol = (sym: vscode.DocumentSymbol | vscode.SymbolInformation, indent = 0) => {
...
export const goToDefinitionTool: ToolSpec = defineTool({
...
export const findReferencesTool: ToolSpec = defineTool({
