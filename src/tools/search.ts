...
export const globSearchTool: ToolSpec = defineTool({
    name: 'glob_search',
    category: 'search',
    description:
        'Fast file pattern matching. Supports glob patterns like "**/*.ts" or "src/**/*.py". ' +
    ...
    async execute(args, ctx) {
        ...
        const include = subPath ? new vscode.RelativePattern(
            vscode.Uri.file(require('path').resolve(ctx.workspaceRoot, subPath)),
            pattern
        ) : pattern;
...
export const grepSearchTool: ToolSpec = defineTool({
    name: 'grep_search',
    category: 'search',
    description:
        'Search file contents using regex patterns. Supports case-insensitive search, ' +
...
export const toolSearchTool: ToolSpec = defineTool({
    name: 'ToolSearch',
    category: 'search',
    description:
        'Search available tool definitions by name or keyword. Returns matching tool names, ' +
    ...
    async execute(args, ctx) {
        ...
        const allTools = ctx.turnState.get('allTools') as ToolSpec[] | undefined;
        if (!allTools) {
        ...
        const scored = allTools.map(t => {
            ...
            if (nameL === query) { score += 100; }
            else if (nameL.includes(query)) { score += 50; }

            const words = query.split(/\s+/);
            for (const w of words) {
                if (nameL.includes(w)) { score += 20; }
                if (descL.includes(w)) { score += 10; }
                if (t.category.includes(w)) { score += 15; }
            }
            return { tool: t, score };
        });
...
export const searchTools: ToolSpec[] = [globSearchTool, grepSearchTool, toolSearchTool];
